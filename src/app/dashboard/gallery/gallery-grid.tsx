      ...
      >
        ...
        <div className="flex flex-col overflow-y-auto p-5">
          ...
          <form action={tagAction} className="mt-4">
            ...
            <button
              type="submit"
              className="mt-2 w-full rounded-md bg-blue-700 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-800"
            >
              Tags speichern
            </button>
            {tagState.error && (
              <p className="mt-1 text-xs text-red-600">{tagState.error}</p>
            )}
            {tagState.ok && (
              <p className="mt-1 text-xs text-green-700">✓ Gespeichert</p>
            )}
          </form>
          ...
          <div className="mt-auto space-y-2 pt-4">
            <a
              ...
              className="block rounded-lg bg-blue-700 px-4 py-2 text-center text-sm font-semibold text-white hover:bg-blue-800"
            >
              ⬇️ Download
            </a>
            {asset.creative_id && (
              <Link
                ...
                className="block rounded-lg border border-blue-700 px-4 py-2 text-center text-sm font-semibold text-blue-700 hover:bg-blue-50"
              >
                → Im Creative öffnen
              </Link>
            )}
            <form action={delAction}>
              ...
              <button
                ...
                className="w-full rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-100"
              >
                🗑️ Löschen
              </button>
              {delState.error && (
                <p className="mt-1 text-xs text-red-600">{delState.error}</p>
              )}
            </form>
          </div>
